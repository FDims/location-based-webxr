...
Plan: `GpsPlusSlamJs_Docs/docs/2026-06-11-depth-occupancy-grid-port-plan.md` §3/Iter 5; AR-space reparenting + 0.1 m cube size in Iter 7.
...
- **`OccupancyGridSource`** — the read surface required of the grid (`getOccupiedCells`, `getCellCenter`); structurally satisfied by the framework's `OccupancyGrid`.
...
4. Color ramp: HSL blue (≤ −1 m) → red (≥ 3 m) over cell height (raw WebXR Y; Up is Y in both frames).
