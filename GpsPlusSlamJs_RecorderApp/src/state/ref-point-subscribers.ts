 ...
 * `selectRefPointEntries` selector (over `state.refPointsV2.entries`).
...
import { selectRefPointEntries } from './ref-points-v2-slice';
import type { RefPointVisualizer } from '../visualization/ref-point-visualizer';

/**
 * Wire the 3D visualizer to the recorder's flat `refPointsV2` slice.
...
export function wireRefPointSubscribers(
  ...
  let last = selectRefPointEntries(store.getState().refPointsV2);
  ...
  return store.subscribe(() => {
    const next = selectRefPointEntries(store.getState().refPointsV2);
    if (next === last) return;
    last = next;
