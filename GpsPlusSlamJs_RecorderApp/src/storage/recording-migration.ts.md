...
- `src/storage/recording-migration.test.ts` — 36 unit tests covering all five eras, GPS field migration, position reversal, malformed payload guards, immutability, empty arrays, null metadata handling, and the Step 5.6 `refPoints` injection pass (insertion shape, idempotency on streams already carrying V2 actions, same-reference contract when no `markReferencePoint`, and interaction with era-1 `gpsPoint→rawGpsPoint` rename).
