...
export function solveClosestPointOfApproach(
  ...
  for (const obs of observations) {
    ...
    if (obs.rayWeight > 0) {
      ...
      A[0] += M[0] * w; A[1] += M[1] * w; A[2] += M[2] * w;
      A[3] += M[3] * w; A[4] += M[4] * w; A[5] += M[5] * w;
      A[6] += M[6] * w; A[7] += M[7] * w; A[8] += M[8] * w;
    ...
    if (obs.depthPoint && obs.depthWeight != null && obs.depthWeight > 0) {
      ...
      A[0] += dx * dx * w; A[1] += dx * dy * w; A[2] += dx * dz * w;
      A[3] += dy * dx * w; A[4] += dy * dy * w; A[5] += dy * dz * w;
      A[6] += dz * dx * w; A[7] += dz * dy * w; A[8] += dz * dz * w;
  ...
  // rmsError: average perpendicular distance via cross-product identity
  let sumSq = 0;
  let validRays = 0;
  for (const obs of observations) {
