...
- `discoverScenariosFromZipMetadata` attaches `h3Cells` to each `SessionEntry` from the `session.json` `h3Cells` field, parsed defensively (`parseH3Cells`: only an array whose entries are _all_ strings is accepted; if any entry is a non-string the whole array is rejected as untrustworthy — all-or-nothing, not silently filtered; a missing/non-array/partially-malformed field yields `undefined`). An empty array is preserved (means "no GPS coverage"), distinct from `undefined` (legacy or corrupt — needs GPS backfill). Consumed by `recording-index.ts` to build the map browser's coverage index.
