...
- **`computeAspectFitSize(cameraWidth, cameraHeight, maxEdge)`** — capture dimensions that **preserve the camera aspect** with the **longer edge fixed at `maxEdge`** (e.g. `512` → `512×384` for a 4:3 camera). Unlike `computeCaptureSize` (divide-by-divisor), this fits a fixed pixel budget — used by the QR blit (B2) so the detector sees an undistorted code. Integer, longer edge `== maxEdge`, each axis ≥ 1. For an invalid `maxEdge` (< 1 / NaN) the edge length falls back to `DEFAULT_BLIT_CONFIG.width` (still aspect-preserving); for invalid camera dims (≤ 0 or NaN) the aspect is unknown → a square at the (possibly defaulted) edge.
