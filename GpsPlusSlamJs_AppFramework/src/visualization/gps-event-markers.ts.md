...
- `addGpsEvent(gpsCoords: [x,y,z], odomPos: [x,y,z], accuracy?: GpsEventAccuracy): void` — add markers for a GPS event.
  ...
  - Half-populated or non-positive accuracy falls back to the legacy fixed sphere (defensive — same policy as `preview-map.ts`).
